...
export function getNameHexByName(config: AvantisConfig, name: string): number[] {
	const chars: number[] = [];
	for (let i = 0; i < name.length; i++) {
		...
		const char = name[i]
		const hex = config.name.char[char];
		if (hex) {
...
export function getColorByHex(config: AvantisConfig, hex: number): string | undefined {

	const item = config.colors.find(x => x.value === hex);
